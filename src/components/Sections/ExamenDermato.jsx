...
import { Activity, Beaker, ChevronLeft, ChevronRight, Edit3, Undo, Redo } from 'lucide-react';
...
function ExamenDermato({ data, updateData }) {
  ...
  const [history, setHistory] = useState([data?.synthese_dermato || '']);
  ...
  useEffect(() => {
    generateSynthesis();
  }, [data?.dermato_checks, data?.dermato_notes]);

  useEffect(() => {
    const currentSynthese = data?.synthese_dermato || '';
  ...
  }, [data?.synthese_dermato]);

  const handleUndo = () => {
    if (historyIndex > 0) {
      const newIndex = historyIndex - 1;
      setHistoryIndex(newIndex);
      updateData({ synthese_dermato: history[newIndex] });
    }
  };
  ...
  const generateSynthesis = () => {
    let parts = [];
    const checks = data?.dermato_checks || {};
    
    const activeChecks = generalCatalogHook.catalog.filter(c => checks[c.id]).map(c => c.label);
    if (activeChecks.length > 0) {
      parts.push(`Signes dermatologiques (Peau & Phanères) : ${activeChecks.join(', ')}.`);
    }

    if (data?.dermato_notes) {
      parts.push(`Notes complémentaires : ${data.dermato_notes}`);
    }

    const draft = parts.join('\n');
    updateData({ synthese_dermato: draft });
  };

  const tabs = [
    { id: 'peau', label: 'Lésions Cutanées', icon: <Activity size={14} /> },
    { id: 'phaneres', label: 'Phanères (Ongles/Cheveux)', icon: <Beaker size={14} /> }
  ];
    ...
    <div className="animate-fade-in glass-panel" style={{ padding: '3rem' }}>
      ...
      {activeTab === 'peau' && (
        <div className="animate-fade-in">
          <CheckboxGridManager 
            categoryName="derm_peau" 
            catalogHook={generalCatalogHook} 
            data={data} 
            updateData={updateData} 
            dataKeyPrefix="dermato_checks"
          />
        </div>
      )}
      ...
      <div style={{ marginTop: '2.5rem', paddingTop: '2.5rem', borderTop: '2px dashed var(--surface-border)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid var(--surface-border)', paddingBottom: '0.5rem' }}>
          <h4 style={{ margin: 0, color: 'var(--primary)' }}>Synthèse Dermatologique</h4>
...
export default React.memo(ExamenDermato);
