...
import { Activity, GitCommit, Heart, ChevronLeft, ChevronRight, Edit3, Undo, Redo } from 'lucide-react';
import { PremiumTextArea } from '../Form/PremiumInput';
import { useDynamicCatalog } from '../../utils/useDynamicCatalog';
import { DEFAULT_CARDIO } from '../../utils/catalogs/cardioCatalog';
import { CheckboxGridManager } from '../UI/CheckboxGridManager';

function ExamenCardio({ data, updateData }) {
  const [activeTab, setActiveTab] = useState('inspection_palpation');
  const scrollContainerRef = useRef(null);

  const cardioCatalogHook = useDynamicCatalog('cardioCatalog', DEFAULT_CARDIO);

  const [history, setHistory] = useState([data?.synthese_cardio || '']);
  const [historyIndex, setHistoryIndex] = useState(0);

  const handleChange = (e) => updateData({ [e.target.id]: e.target.value });

  // Auto-génération de la synthèse
  useEffect(() => {
    generateSynthesis();
  }, [data?.cardio_checks, data?.cardio_notes]);

  useEffect(() => {
    const currentSynthese = data?.synthese_cardio || '';
  ...
  }, [data?.synthese_cardio]);

  const handleUndo = () => {
    if (historyIndex > 0) {
      const newIndex = historyIndex - 1;
      setHistoryIndex(newIndex);
      updateData({ synthese_cardio: history[newIndex] });
    }
  };
  ...
  const generateSynthesis = () => {
    let parts = [];
    const checks = data?.cardio_checks || {};
    
    const activeChecks = cardioCatalogHook.catalog.filter(c => checks[c.id]).map(c => c.label);
    if (activeChecks.length > 0) {
      parts.push(`Signes cardio-vasculaires : ${activeChecks.join(', ')}.`);
    }

    if (data?.cardio_notes) {
      parts.push(`Notes complémentaires : ${data.cardio_notes}`);
    }

    const draft = parts.join('\n');
    updateData({ synthese_cardio: draft });
  };

  const tabs = [
    { id: 'inspection_palpation', label: 'Inspection & Palpation', icon: <Heart size={14} /> },
    { id: 'percussion', label: 'Percussion', icon: <Activity size={14} /> },
    { id: 'auscultation', label: 'Auscultation', icon: <GitCommit size={14} /> },
    { id: 'vaisseaux', label: 'Vaisseaux (Artères & Veines)', icon: <Activity size={14} /> }
  ];
  ...
  return (
    <div className="animate-fade-in glass-panel" style={{ padding: '3rem' }}>
      ...
      {activeTab === 'inspection_palpation' && (
        <div className="animate-fade-in">
          <h3 style={{ color: 'var(--primary)', marginBottom: '1rem', borderBottom: '1px dashed var(--surface-border)', paddingBottom: '0.5rem' }}>Inspection de l'aire précordiale et Palpation</h3>
          <CheckboxGridManager 
            categoryName="cardio_inspection" 
            catalogHook={cardioCatalogHook} 
            data={data} 
            updateData={updateData} 
            dataKeyPrefix="cardio_checks"
          />
        </div>
      )}
      ...
      {activeTab === 'percussion' && (
        <div className="animate-fade-in">
          <h3 style={{ color: 'var(--primary)', marginBottom: '1rem', borderBottom: '1px dashed var(--surface-border)', paddingBottom: '0.5rem' }}>Percussion de l'aire cardiaque</h3>
          <CheckboxGridManager 
            categoryName="cardio_percussion" 
            catalogHook={cardioCatalogHook} 
            data={data} 
            updateData={updateData} 
            dataKeyPrefix="cardio_checks"
      ...
      {activeTab === 'auscultation' && (
        <div className="animate-fade-in">
          <h3 style={{ color: 'var(--primary)', marginBottom: '1rem', borderBottom: '1px dashed var(--surface-border)', paddingBottom: '0.5rem' }}>Auscultation Cardiaque</h3>
          <CheckboxGridManager 
            categoryName="cardio_auscultation" 
            catalogHook={cardioCatalogHook} 
            data={data} 
            updateData={updateData} 
            dataKeyPrefix="cardio_checks"
      ...
      {activeTab === 'vaisseaux' && (
        <div className="animate-fade-in">
          <h3 style={{ color: 'var(--primary)', marginBottom: '1rem', borderBottom: '1px dashed var(--surface-border)', paddingBottom: '0.5rem' }}>Artères et Veines</h3>
          <CheckboxGridManager 
            categoryName="cardio_vaisseaux" 
            catalogHook={cardioCatalogHook} 
            data={data} 
            updateData={updateData} 
            dataKeyPrefix="cardio_checks"
      ...
      <div style={{ marginTop: '2.5rem', paddingTop: '2.5rem', borderTop: '2px dashed var(--surface-border)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid var(--surface-border)', paddingBottom: '0.5rem' }}>
          <h4 style={{ margin: 0, color: 'var(--primary)' }}>Synthèse de l'Examen Cardio-Vasculaire</h4>
...
export default React.memo(ExamenCardio);
