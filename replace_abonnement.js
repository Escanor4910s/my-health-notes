const fs = require('fs');
const content = fs.readFileSync('src/components/Screens/Dashboard.jsx', 'utf8');

const abonnement_ui =               {activeTab === 'abonnement' && (
...
const startStr = "{activeTab === 'abonnement' && (";
const endStr = "{activeTab === 'parametres' && (";

const startIdx = content.indexOf(startStr);
const endIdx = content.indexOf(endStr);

if (startIdx !== -1 && endIdx !== -1) {
    const chunkBeforeParametres = content.substring(startIdx, endIdx);
    const lastBrace = chunkBeforeParametres.lastIndexOf(")}");
    
    if (lastBrace !== -1) {
        let newContent = content.substring(0, startIdx) + abonnement_ui + "\n              " + content.substring(startIdx + lastBrace + 2);
        
        // Remove mobile logout wrapper
        newContent = newContent.replace(/<div className="mobile-logout-wrapper"[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*<\/>/, '</div>\n          </div>\n        </div>\n      </>');
        
        fs.writeFileSync('src/components/Screens/Dashboard.jsx', newContent, 'utf8');
        console.log("Success");
    }
}
